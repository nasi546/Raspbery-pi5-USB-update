...
문제가 생기면 **이전 버전으로 자동 롤백**되는 예제입니다.

현업에서 자주 쓰는 **backup → pending → health → rollback** 흐름을  
...
- **History Log 기록**
  - `/var/log/usb-updater-history.log`에 한 줄 요약 기록  
    (timestamp / status / version / message)
...
Raspberry Pi OS 환경 + /opt/my-app + UPDATE_USB 라벨 기준
...
<details> <summary><b>A) “USB 꽂았는데 업데이트가 시작 자체를 안 함”</b></summary>
Symptom
...
```
</details> <details> <summary><b>B) “서비스는 떴는데 manifest/app.tar.gz를 못 찾음”</b></summary>
Symptom
...
```
</details> <details> <summary><b>C) “업데이트 직후 다시 롤백됨 (오동작처럼 보임)”</b></summary>
Symptom
...
</details> <details> <summary><b>D) “업데이트가 계속 skip 처리됨”</b></summary>

Symptom
새 버전을 넣었다고 생각했는데 설치가 안 되고 skip 로그만 남음

Cause
manifest.json의 version이 state.json의 current_version과 동일
...
tail -n 30 /var/log/usb-updater-history.log
```
</details> <details> <summary><b>E) “수동 롤백이 안 됨 / 백업이 없다”</b></summary>
...
</details> <details> <summary><b>F) “sha256 필드가 있는데 무결성 검증이 약한 것 같음”</b></summary>
